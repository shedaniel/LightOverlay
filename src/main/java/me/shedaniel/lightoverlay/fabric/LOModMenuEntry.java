...
public class LOModMenuEntry implements ModMenuApi {
    ...
    public Screen getConfigScreenByCloth(Screen parent) {
        ...
        general.addEntry(eb.startIntSlider(new TranslatableText("config.lightoverlay.crossLevel"), LightOverlay.crossLevel, 0, 15).setDefaultValue(7).setTextGetter(integer -> new LiteralText("Level: " + integer)).setSaveConsumer(integer -> LightOverlay.crossLevel = integer).build());
        general.addEntry(eb.startIntSlider(new TranslatableText("config.lightoverlay.optimalPlacementLevel"), LightOverlay.optimalPlacementLevel, -1, 15).setDefaultValue(1).setTextGetter(integer -> new LiteralText(integer < 0 ? "Off" : "Level: " + integer)).setSaveConsumer(integer -> LightOverlay.optimalPlacementLevel = integer).build());
