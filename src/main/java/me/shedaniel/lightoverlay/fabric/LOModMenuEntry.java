...
public class LOModMenuEntry implements ModMenuApi {
    ...
    public Screen getConfigScreenByCloth(Screen parent) {
        ...
        general.addEntry(eb.startTextDescription(new TranslatableText("description.lightoverlay.caching").formatted(Formatting.GRAY)).build());
        ...
        IntegerSliderEntry crossLevel = eb.startIntSlider(new TranslatableText("config.lightoverlay.crossLevel"), LightOverlay.crossLevel, 0, 15).setDefaultValue(7).setTextGetter(integer -> new LiteralText("Level: " + integer)).setSaveConsumer(integer -> LightOverlay.crossLevel = integer).build();
